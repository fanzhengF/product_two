...
$.Autocompleter = function(input, options) {
	...
	function selectCurrent(event) {
		...
		if(event.keyCode==13 && typeof (options.enterFun)=="function"){
            options.enterFun(selected.data);
        }
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	function fillList() {
		...
		for (var i=0; i < max; i++) {
			...
			if ( formatted === false )
				continue;
			var li = $("<li/>").data(data[i].data).html( options.highlight(formatted, term) ).addClass(i%2 == 0 ? "ac_even" : "ac_odd").appendTo(list)[0];
            if(typeof(options.txtClick)=="function"){
                $(li).click(function(){
                    options.txtClick($(this).data());
                })
            }
