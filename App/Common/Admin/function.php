...
function is_email($value) {
    return preg_match("/^[0-9a-zA-Z]+(?:[\_\.\-][a-z0-9\-]+)*@[a-zA-Z0-9\_]+(?:[-.][a-zA-Z0-9]+)*\.[a-zA-Z]+$/i", $value);
}
...
function checkMobile($mobile){
	$check = preg_match('/^(13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])\d{8}$/', $mobile);
	return $check;
}

/*
 * 为mongodb内容中的user_id,target_id的身份做区分
 * o开头，为用户
 * return true 用户   false 工长、监理
 **/
function checkUserIdentity($id){
    if(substr($id,0,1) == 'o')  return true;
    return false;
}

/*
 * 为mongodb内容中的send_time的毫秒级时间戳，转换为秒级时间戳，并格式化输出
 **/
function sendTimeFormat($send_time){
    return date('Y年m月d日 H:i:s',substr($send_time,0,10));
}
/**
*
...
* @return boolean
*/
function userAuth($GCA, $uid = FALSE){
	import('UserLogic');
	return UserLogicModel::getAuth($GCA, $uid);
}
 ...
 * @return string
 */
function getPicUrl($logo) {
    return C('BASE_PIC_URL').$logo;
}
