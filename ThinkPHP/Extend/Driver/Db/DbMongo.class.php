...
class DbMongo extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            try{
                //$this->linkID[$linkNum] = new mongo( $host,(array)$config['params']);
                $this->linkID[$linkNum] = new mongo( $host );
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        try{
            // 记录开始执行时间
            G('queryStartTime');
            $result =  $replace?   $this->_collection->save($data,true):  $this->_collection->insert($data);
            //$this->debug();
            if($result) {
               $_id    = $data['_id'];
                if(is_object($_id)) {
