 ...
 * @version $Id: ImageMagickwand.class.php 498412 2013-07-16 07:31:22Z liyue1 $
 */
class ImageMagickwand {
    /**
     * 图像资源对象
     * @var resource
     */
    ...
    public function open($imgname){
        ...
        //销毁已存在的图像
        empty($this->img) || DestroyMagickWand($this->img);

        //载入图像
        $this->img = NewMagickWand();
        MagickReadImage($this->img, $imgname);
        // magickwand 函数获取gif图宽度不准确
    ...
    public function save($imgname, $type = null, $interlace = true){
        ...
        if(is_null($type)){
            ...
            MagickSetImageFormat($this->img, $type);
        }

        //JPEG图像设置隔行扫描
        if('jpeg' == $type || 'jpg' == $type){
            MagickSetInterlaceScheme($this->img, 1);
        }

        //去除图像配置信息
        MagickStripImage($this->img);
        MagickSetCompressionQuality($this->img,100);
        //保存图像
        $imgname = realpath(dirname($imgname)) . '/' . basename($imgname); //强制绝对路径
        if ('gif' == $type) {
            MagickWriteImages($this->img, $imgname, true);
        } else {
            MagickWriteImage($this->img, $imgname);
        }
    }
    ...
    public function crop($w, $h, $x = 0, $y = 0, $width = null, $height = null){
        ...
        if('gif' == $this->info['type']){
            $img = MagickCoalesceImages($this->img);
            DestroyMagickWand($this->img); //销毁原图
            ...
            //压缩图片
            $this->img = MagickDeconstructImages($img);
            DestroyMagickWand($img); //销毁零时图片
        } else {
...
//            $img->setImagePage($w, $h, 0, 0); //调整画布和图片一致
    ...
    public function thumb($width, $height, $type){
        ...
        switch ($type) {
            ...
            case THINKIMAGE_THUMB_FILLED:
                ...
                //创建一张新图像
                $newimg = NewMagickWand();
                MagickNewImage($newimg, $width, $height, 'white');
                MagickSetImageType($newimg, MagickGetImageType($this->img));


                if('gif' == $this->info['type']){
                    $imgs = MagickCoalesceImages($this->img);
                    $img  = new NewMagickWand();
                    DestroyMagickWand($this->img); //销毁原图
                    ...
                    do {
                        ...
                        MagickAddImage($img, $image);
                        MagickSetImageDelay($img, MagickGetImageDelay($imgs));

                        DestroyMagickWand($image); //销毁零时图片

                    } while (MagickNextImage($imgs));

                    //压缩图片
                    DestroyMagickWand($this->img);
                    $this->img = MagickDeconstructImages($img);
                    DestroyMagickWand($imgs); //销毁零时图片
                    DestroyMagickWand($img); //销毁零时图片
            ...
            /* 短边缩略 */
            case THINKIMAGE_THUMB_FULL_1:
                $x = $y = 0;
                //缩略图的宽高比例
                $scale_t = $width/$height;
                //原图宽高比例
                $scale_o = $w/$h;
                if($scale_t > $scale_o) {
                    //宽是短边
    ...
    private function _fill($newimg, $posx, $posy, $neww, $newh, $img = null){
        ...
        /* 将指定图片绘入空白图片 */
        $draw  = NewDrawingWand();
        DrawComposite($draw, MagickGetImageCompose($img), $posx, $posy, $neww, $newh, $img);
        $image = CloneMagickWand($newimg);
	 ...
	 * @param  integer $pad 离边线的距离，防止贴边
     */
    public function water($source, $locate = THINKIMAGE_WATER_SOUTHEAST, $pad = 0){
        //资源检测
        if(empty($this->img)) throw new Exception('没有可以被添加水印的图像资源');
        if(!is_file($source)) throw new Exception('水印图像不存在');

        //创建水印图像资源
        MagickResetIterator($this->img);
        $water = NewMagickWand();
        MagickReadImage($water, realpath($source));
        $info  = array(MagickGetImageWidth($water), MagickGetImageHeight($water));

        /* 设定水印位置 */
        switch ($locate) {
            /* 右下角水印 */
            case THINKIMAGE_WATER_SOUTHEAST:
                $x = $this->info['width'] - $info[0] - $pad;
                $y = $this->info['height'] - $info[1] - $pad;
                break;
        ...
        do{
            MagickCompositeImage($this->img, $water, MW_OverCompositeOp, $x, $y);
        }while(MagickNextImage($this->img));
        DestroyMagickWand($water);
}
    ...
    public function text($text, $font, $size, $color = '#00000000', 
        ...
        //获取文字信息
        $draw = NewDrawingWand();
        DrawSetFont($draw, realpath($font));
        DrawSetFontSize($draw, $size);
        DrawSetFillColor($draw, $col);
        DrawSetFillAlpha($draw, 1-hexdec($alp)/127);
        DrawSetTextAntialias($draw, true);
        DrawSetStrokeAntialias($draw, true);
        
        $metrics = MagickQueryFontMetrics($this->img, $draw, $text);
        ...
        if('gif' == $this->info['type']){
            ...
            DestroyMagickWand($this->img); //销毁原图
            do{
                MagickAnnotateImage($img, $draw, $x + $ox, $y + $oy, $angle, $text);
            } while (MagickNextImage($img));
            ...
            MagickAnnotateImage($this->img, $draw, $x + $ox, $y + $oy, $angle, $text);
        }
        DestroyDrawingWand($draw);
    }
