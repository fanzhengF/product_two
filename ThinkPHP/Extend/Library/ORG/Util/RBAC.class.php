...
class RBAC {
	...
	//检查当前操作是否需要认证
	static function checkAccess($MODULE_NAME='',$ACTION_NAME='') {
	...
	//权限认证的过滤器方法
	static public function AccessDecision($appName = APP_NAME, $modelName = FALSE, $actionName = FALSE, $uid = FALSE) {
		//检查是否需要认证

		if (RBAC::checkAccess($modelName,$actionName)) {
			//存在认证识别号，则进行进一步的访问决策
			$MODULE_NAME = $modelName ? $modelName : MODULE_NAME;
			$ACTION_NAME = $actionName ? $actionName : ACTION_NAME;
			...
			if (empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
				if ($uid ) {
					...
					//通过数据库进行访问检查
					$AccessUid = $uid ;
					$accessList = RBAC::getAccessList($AccessUid);
				} else {
...
//				$module = defined('P_MODULE_NAME') ? P_MODULE_NAME : MODULE_NAME;
	...
	static public function getAccessList($authId) {
		...
		$sql = "select node.id,node.name from " .
				...
				$table['node'] . " as node " .
				"where user.id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=1 and node.status=1";
		...
		foreach ($apps as $key => $app) {
			...
			$sql = "select node.id,node.name from " .
					...
					$table['access'] . " as access ," .
					$table['node'] . " as node " .
					"where user.id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$appId} and node.status=1";
			...
			foreach ($modules as $key => $module) {
				...
				if ('PUBLIC' == strtoupper($moduleName)) {
					$sql = "select node.id,node.name from " .
							...
							$table['access'] . " as access ," .
							$table['node'] . " as node " .
							"where user.id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$moduleId} and node.status=1";
					$rs = $db->query($sql);
					foreach ($rs as $a) {
